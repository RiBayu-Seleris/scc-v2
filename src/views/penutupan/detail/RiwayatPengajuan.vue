...
onMounted(async () => {
  try {
    ...
    // Profil wajib tersedia karena role menentukan aksi catatan dan notifikasi.
    if (!user) return
...
async function submit() {
  ...
  saving.value = true
  try {
    await storeSubmissionHistory({
      submission_id: parseInt(id, 10),
      user_id: parseInt(session.userId, 10),
      description: description.value,
    })
    histories.value = await getSubmissionHistory(id)
    description.value = ''
  } catch {
