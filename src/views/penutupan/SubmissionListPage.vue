...
import { submissionListFetcher, confirmSubmissionDebet } from '@/lib/services/submission'
...
const takafulPengajuanTabs = [
  ...
  { label: 'Pengajuan Belum Lengkap', route: 'takaful-pengajuan-belum-lengkap' },
]
...
const viewConfigs = {
  'pengajuan-otomatis': { title: 'List Pengajuan', filter: { submission: 'fcl' }, group: 'pengajuan', columns: 'pengajuan-otomatis' },
  ...
  'takaful-pengajuan-lengkap': { title: 'List Pengajuan', filter: { insurance_company_id: TAKAFUL_INSURANCE_COMPANY_ID, acceptance: 'onreview' }, group: 'takaful-pengajuan', columns: 'pengajuan' },
  'takaful-pengajuan-belum-lengkap': { title: 'List Pengajuan', filter: { insurance_company_id: TAKAFUL_INSURANCE_COMPANY_ID, acceptance: 'unrisked' }, group: 'takaful-pengajuan', columns: 'pengajuan' },
  'takaful-keputusan-diterima': { title: 'List Keputusan', filter: { insurance_company_id: TAKAFUL_INSURANCE_COMPANY_ID, acceptance: 'accepted' }, group: 'takaful-keputusan', columns: 'pengajuan' },
  'takaful-keputusan-ditolak': { title: 'List Keputusan', filter: { insurance_company_id: TAKAFUL_INSURANCE_COMPANY_ID, acceptance: 'rejected' }, group: 'takaful-keputusan', columns: 'pengajuan' },

  'underwriting': { title: 'List Data Underwriting', filter: { submission: 'fcl' }, group: 'underwriting', columns: 'underwriting', constantStatus: 'Menunggu Konfirmasi Broker' },
  'underwriting-non-medis': { title: 'List Data Underwriting', filter: { acceptance: 'accepted', submission: 'non-medis' }, group: 'underwriting', columns: 'underwriting', constantStatus: 'Menunggu Konfirmasi Broker' },
  'underwriting-medis': { title: 'List Data Underwriting', filter: { acceptance: 'accepted', submission: 'medis' }, group: 'underwriting', columns: 'underwriting', constantStatus: 'Menunggu Konfirmasi Broker' },

  'keputusan': { title: 'List Keputusan', filter: { acceptance: 'accepted' }, group: 'keputusan', columns: 'keputusan-diterima' },
  ...
  'pengajuan-inforce': { title: 'List Pengajuan Inforce', filter: { acceptance: 'accepted' }, roleFilters: { Management: { already_recon: true, payment_status: 'paid_by_recon' } }, group: 'status', columns: 'keputusan-status' },
  'pengajuan-pending': { title: 'List Pengajuan Pending', filter: { acceptance: 'accepted' }, roleFilters: { Management: { already_recon: true, payment_status: 'paid' } }, group: 'status', columns: 'keputusan-status' },
  'pengajuan-outstanding': { title: 'List Pengajuan Outstanding', filter: { acceptance: 'accepted' }, roleFilters: { Management: { already_recon: false } }, group: 'status', columns: 'keputusan-status' },
  ...
  'pembatalan-nomor-sertifikat': { title: 'Pembatalan Nomor Sertifikat', filter: { acceptance: 'cancelled' }, group: 'pembatalan', columns: 'pembatalan-sertifikat', constantStatus: 'Menunggu Konfirmasi Broker' },

  'cover-note': { title: 'List Cover Note', filter: { acceptance: 'accepted', payment: 'paid' }, group: 'cover-note', columns: 'cover-note' },
  'cover-note-belum-bayar': { title: 'List Cover Note Belum Bayar', filter: { acceptance: 'accepted', payment: 'unpaid' }, group: 'cover-note', columns: 'cover-note' },
...
const tabs = computed(() => {
  switch (config.value.group) {
    case 'pengajuan': return penutupanTabs
    case 'underwriting': return underwritingTabs
    case 'keputusan': return keputusanTabs
    case 'status': return statusTabs
    case 'cover-note': return coverNoteTabs
    case 'debit-note': return debitNoteTabs
    case 'pembatalan': return pembatalanTabs
    case 'takaful-pengajuan': return takafulPengajuanTabs
    case 'takaful-keputusan': return takafulKeputusanTabs
    default: return []
  }
})
...
const baseColumns = {
  ...
  broker: { key: 'insurance_broker', label: 'Nama Broker', hidden: ![26, 27, 28, 29].includes(partnerId) },
  broker26: { key: 'insurance_broker', label: 'Nama Broker', hidden: partnerId !== 26 },
  ...
  pengajuanNumberAlways: { key: 'submission_identifier', label: submissionIdentifierLabel(), formatter: (_, row) => submissionIdentifier(row) },
  pengajuanOtomatisNumber: { key: 'submission_identifier', label: 'No. Pengajuan Kredit', formatter: (_, row) => automaticSubmissionIdentifier(row) },
...
function columnsFor(type) {
  ...
  if (type === 'pengajuan-otomatis') {
    return [
      baseColumns.aksi, baseColumns.pengajuanOtomatisNumber, baseColumns.identitas,
      baseColumns.member, baseColumns.debitur, baseColumns.produk, baseColumns.usia,
      baseColumns.period, baseColumns.sum, baseColumns.basic, baseColumns.extra,
      baseColumns.total, baseColumns.status, baseColumns.created,
    ]
  }
  if (type === 'underwriting') {
    return [
      baseColumns.aksi, baseColumns.broker26, baseColumns.pengajuanNumberAlways, baseColumns.ehd,
  ...
  if (type === 'keputusan-diterima') {
    return [...keputusan.slice(0, -2), baseColumns.rate, baseColumns.status, baseColumns.created]
      .filter((c) => !c.hidden)
  }
  if (type === 'keputusan-status') {
    return [
      baseColumns.aksi, baseColumns.broker26, baseColumns.pengajuanNumber,
      baseColumns.ehd, baseColumns.kodeAo, baseColumns.akad, baseColumns.nik,
      baseColumns.member, baseColumns.debitur, baseColumns.produk, baseColumns.usia,
      baseColumns.period, baseColumns.start, baseColumns.end, baseColumns.sum,
      baseColumns.basic, baseColumns.extra, baseColumns.rate, baseColumns.status,
      baseColumns.created,
    ].filter((c) => !c.hidden)
  }
  if (type === 'pembatalan') {
    return [
      ...keputusan.slice(0, -2), baseColumns.total, baseColumns.status,
      baseColumns.created,
    ].filter((c) => !c.hidden)
  }
  ...
  if (type === 'cover-note') {
    return [
      baseColumns.aksi, baseColumns.broker26, baseColumns.pengajuanNumberAlways,
      baseColumns.ehd, baseColumns.kodeAo, baseColumns.identitas, baseColumns.kantor,
      baseColumns.debitur, baseColumns.produk, baseColumns.usia, baseColumns.period,
      baseColumns.start, baseColumns.end, baseColumns.sum, baseColumns.basic,
      baseColumns.extra, baseColumns.status, baseColumns.created,
    ].filter((c) => !c.hidden)
  ...
  if (type === 'debit-note') {
    return [
      ...
      baseColumns.aksi, baseColumns.pengajuanNumberAlways, baseColumns.ehd,
      baseColumns.kodeAo, baseColumns.identitas, baseColumns.kantor,
      baseColumns.debitur, baseColumns.produk, baseColumns.usia, baseColumns.period,
      baseColumns.start, baseColumns.end, baseColumns.sum, baseColumns.basic,
      baseColumns.extra, baseColumns.status, baseColumns.created,
    ].filter((c) => !c.hidden)
...
const fetcher = computed(() => submissionListFetcher(activeFilter.value, config.value.roleFilters))
const tableKey = computed(() => `${props.view || route.name}:${JSON.stringify(activeFilter.value)}:${JSON.stringify(config.value.roleFilters || {})}`)
...
<template>
  <div>
    ...
    <Card no-body class="p-4">
      ...
      >
        <template #toolbar>
          <BaseButton v-if="config.selectable && selectedIds.length" variant="success" :loading="confirming" @click="confirmDebet">
        ...
        <template #header-select="{ rows }">
          <input
            ...
            :checked="rows.length > 0 && rows.every((row) => selectedIds.includes(Number(row.id)))"
        ...
        <template #cell-acceptance_status_description="{ value }">
          <Badge :variant="statusVariant(displayStatus(value))">{{ displayStatus(value) }}</Badge>
        </template>
      </DataTable>
    </Card>
  </div>
